...
public class LoginServlet extends HttpServlet {  //需要继承HttpServlet  并重写doGet  doPost方法
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String customerId = request.getParameter("customerId"); //得到jsp页面传过来的参数
        String loginPassword = request.getParameter("loginPassword");

        UserDao user = new UserDaoImpl();

        if(user.Login(customerId, loginPassword)){
            request.setAttribute("xiaoxi", "欢迎用户" + customerId); //向request域中放置信息
